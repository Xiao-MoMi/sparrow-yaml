...
public interface ParentNode<T extends YamlNode<?>> {
     ...
     * @return 创建完成的叶子节点
     */
    default YamlNode<?> setNode(@NotNull Route route, @Nullable Object value) {
        Objects.requireNonNull(route, "路由不可为 null !");

        YamlNode<?> currentNode = this.yamlNode();
        Route currentRoute = this.yamlNode().route();
        YamlDocument root = currentNode.root();
        StandardRepresenter representer = root.sparrowYaml().standardRepresenter();
            ...
            if (currentNode instanceof SequenceNode sequenceNode) {
                ...
                if (targetNode == null) {
                    ...
                    if (isLastElement) {
                        ...
                        Node keyNode = representer.represent(nextRouteKey);
                        Route newRoute = Route.addTo(currentRoute, nextRouteKey);

                        // 第一种情况, 是 IndexElement, 则建立 SequenceNode;
                        if (createRouteElement instanceof IndexElement) {
                            targetNode = new SequenceNode(root, sequenceNode, newRoute, keyNode, new ArrayList<>());
                        }
                        // 第二种情况, 是 KeyElement, 则建立 SectionNode;
                        else if (createRouteElement instanceof KeyElement) {
                            targetNode = new SectionNode(root, sequenceNode, newRoute, keyNode, new LinkedHashMap<>());
                        }
                    }
                }
            ...
            // 如果当前节点是 Section 节点, 则进行搜索获取下一个节点;
                ...
                if (targetNode == null) {
                    ...
                    if (isLastElement) {
                        ...
                        Node keyNode = representer.represent(nextRouteKey);
                        Route newRoute = Route.addTo(currentRoute, sectionNode.adaptKey(nextRouteKey));

                        // 第一种情况, 是 IndexElement, 则建立 SequenceNode;
                        if (createRouteElement instanceof IndexElement) {
                            targetNode = new SequenceNode(root, sectionNode, newRoute, keyNode, new ArrayList<>());
                        }
                        // 第二种情况, 是 KeyElement, 则建立 SectionNode;
                        else if (createRouteElement instanceof KeyElement) {
                            targetNode = new SectionNode(root, sectionNode, newRoute, keyNode, new LinkedHashMap<>());
                        }
                    }
                }
